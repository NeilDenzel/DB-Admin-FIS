...
@section('title', 'Cursos - FIS Admin')
...
<div class="bg-white rounded-3xl border border-slate-200 shadow-[0_15px_15px_-10px_rgba(133,189,215,0.88)] overflow-hidden transition-all duration-300 hover:shadow-[0_25px_25px_-15px_rgba(133,189,215,0.88)]">
        ...
        <table class="min-w-full text-sm">
            <thead class="bg-blue-50/50">
                <tr>
                    ...
                    <th class="text-center px-4 py-3 font-semibold text-slate-700">Tipo</th>
                    <th class="text-left px-4 py-3 font-semibold text-slate-700">Malla</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                    <td class="px-4 py-3 font-mono text-xs text-slate-600">{{ $c->cod_curso }}</td>
                    <td class="px-4 py-3 text-slate-800 max-w-[200px] truncate">{{ $c->nombre }}</td>
                    <td class="px-4 py-3 text-center text-slate-600">{{ $c->ciclo }}°</td>
                    <td class="px-4 py-3 text-center text-slate-600">{{ $c->creditos }}</td>
                    <td class="px-4 py-3 text-center">
                        <span class="inline-block px-2.5 py-1 text-xs font-medium rounded-lg {{ $c->tipo == 'Obligatorio' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700' }}">
                            {{ $c->tipo }}
                        </span>
                    </td>
                    <td class="px-4 py-3 text-slate-500">{{ $c->malla_nombre ?? '—' }}</td>
                    <td class="px-4 py-3 text-center">
                        <div class="flex items-center justify-center gap-2">
                            <a href="{{ url('admin/cursos/' . $c->cod_curso . '/editar') }}"
                            ...
                            <form method="POST" action="{{ url('admin/cursos/' . $c->cod_curso) }}" onsubmit="return confirm('¿Eliminar este curso?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                ...
                @empty
                <tr>
                    <td colspan="7" class="text-center py-12 text-slate-400">No hay cursos registrados.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="px-6 py-3 bg-blue-50/30 border-t border-slate-100 text-xs text-slate-500">
        Total: <strong>{{ $cursos->total() }}</strong> cursos
    </div>
</div>
